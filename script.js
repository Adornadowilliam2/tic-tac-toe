let currentPlayer = "X";
let gameOver = false;
let easyMode = false; // Flag for easy mode
...
function makeAIMove() {
  ...
  for (let i = 0; i < cells.length; i++) {
    if (cells[i].innerHTML === "") {
      ...
      if (easyMode) {
        // Simpler AI logic for easy mode
        score = Math.random(); // Random score for each move
      }
...
function easy() {
  easyMode = !easyMode;
  resetGame(); // Reset the game to apply easy mode immediately
}
